...
Comece estendendo `GraphHelper` a classe para adicionar uma função para obter os eventos do usuário. Abra o arquivo **GraphHelper** e adicione as seguintes `import` instruções à parte superior do arquivo.
...
Adicione as seguintes funções à `GraphHelper` classe.
...
Agora atualize `CalendarFragment` para usar essas novas funções. Adicione as seguintes `import` instruções à parte superior do arquivo **CalendarFragment** .
...
Adicione os membros a seguir à `CalendarFragment` classe.
...
Agora, adicione as seguintes funções à `CalendarFragment` classe para ocultar e mostrar a barra de progresso e para fornecer um retorno de chamada `getEvents` para a `GraphHelper`função no.
...
Por fim, substitua `onCreate` a função na `GraphHelper` classe para obter os eventos do usuário do Microsoft Graph.
...
Agora você pode substituir o despejo JSON por algo para exibir os resultados de forma amigável. Comece substituindo o `TextView` em **app/res/layout/fragment_calendar. xml** por um `ListView`.
...
Em seguida, crie um layout para cada item no `ListView`. Clique com o botão direito do mouse na pasta **app/res/layout** e escolha **novo**e, em seguida, **arquivo de recurso de layout**. Nomeie o arquivo `event_list_item`, altere o **elemento raiz** para `RelativeLayout`e escolha **OK**. Abra o arquivo **event_list_item. xml** e substitua seu conteúdo pelo seguinte.
...
Agora, crie um adaptador de lista personalizado `ListView`para o. Isso é necessário para criar os modos de exibição para os itens da lista e mapear os campos de cada `Event` um para o `TextView` apropriado no modo de exibição.

Clique com o botão direito do mouse na pasta **app/Java/com. example. graphtutorial** e escolha **nova**e, em seguida, **classe Java**. Nomeie a classe `EventListAdapter` e escolha **OK**. Abra o arquivo **EventListAdapter** e substitua seu conteúdo pelo seguinte.
...
Por fim, atualize `CalendarFragment` a classe para usar `EventListAdapter` o para inicializar `ListView`o. Abra a classe **CalendarFragment** e adicione a função a seguir à classe.
...
Adicione a seguinte linha de código na `success` substituição após a `mEventList = iEventCollectionPage.getCurrentPage();` linha.
